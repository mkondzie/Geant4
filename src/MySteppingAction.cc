...
void MySteppingAction::UserSteppingAction(const G4Step* step)
{
   ...
   // G4double previousEnergy = preStepPoint->GetKineticEnergy();
    ...
    if (process && process->GetProcessName() == "compt") {
        ...
        //G4cout << preMomentum.mag() << "  pre" << G4endl;
        ...
        G4cout << parentID << "  parent ID" << G4endl;
        G4EventManager* eventManager = G4EventManager::GetEventManager();
        const G4Event* currentEvent = eventManager->GetConstCurrentEvent();
        const G4PrimaryParticle* parentParticle = currentEvent->GetPrimaryVertex(parentID)->GetPrimary(parentID);
        G4ThreeVector preMomentum = parentParticle->GetMomentum();
        G4cout << preMomentum.mag() << "  pre" << G4endl;

        const G4DynamicParticle *daughtherParticle = track->GetDynamicParticle();
        G4ThreeVector postMomentum = daughtherParticle->GetMomentum();
        G4cout << postMomentum.mag() << "  post" << G4endl;
        G4PrimaryParticle* parentParticle = daughtherParticle->GetPrimaryParticle();
        G4ThreeVector preMomentum = parentParticle->GetMomentum();
        */
        ...
        //else {
            ...
            if (cosTheta >= 1 || cosTheta <= -1) {
                ...
                G4double maxScatteringAngle = 0.002 * rad; //0.0001
                ...
                if (scatteringAngle > maxScatteringAngle) {
                    ...
                    //man->FillNtupleDColumn(1, 3, preEnergy / MeV);
                    //man->FillNtupleDColumn(1, 4, postEnergy / MeV);
